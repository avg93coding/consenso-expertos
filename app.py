...
if "session" in params:
    ...
    st.title("🗳️ Sistema de Votación Digital")

    # Información del participante
    with st.form("form_datos_participante"):
        st.subheader("👤 Datos del participante")
        name = st.text_input("Nombre completo", help="Nombre real del votante")
        correo = st.text_input("Correo electrónico") if es_privada else None
        continuar = st.form_submit_button("Continuar")

    if not continuar or not name:
        st.stop()

    if es_privada and (not correo or not correo_autorizado(correo, code)):
        st.error("El correo no está autorizado para esta sesión privada.")
        st.stop()

    ya_voto = (name in s["names"]) if tipo == "STD" else False
    if ya_voto:
        st.success("✅ Ya registró su voto en esta sesión.")
        st.stop()

    if tipo == "STD":
        st.subheader("📋 Recomendaciones")
        lista_recos = re.split(r'\s*\d+\.\s*', s["desc"])
        lista_recos = [r.strip() for r in lista_recos if r.strip()]
        ...
        voto = st.radio(
            "Seleccione su nivel de acuerdo con el paquete completo de recomendaciones:",
            options=list(range(1, 10)),
            format_func=lambda x: f"{x} - {'En desacuerdo' if x <= 3 else 'Neutral' if x <= 6 else 'De acuerdo'}",
            horizontal=True
        )

        comentario = st.text_area("Comentario adicional (opcional)")
        acepta_terminos = st.checkbox("Confirmo que he leído las recomendaciones y emito mi voto de forma libre")

        if st.button("✅ Enviar mi voto", disabled=not acepta_terminos):
            pid = hashlib.sha256(name.encode()).hexdigest()[:8]
            s["names"].append(name)
            s["ids"].append(pid)
            s["votes"].append(voto)
